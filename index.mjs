    ...
    this.server = net.createServer((socket) => {
      ...
      socket.on("data", (data) =>
        data
          .split(DELIMITER)
          .slice(0, -1)
          .forEach((task) => this.emit("message", JSON.parse(task), socket))
      );
...
class Multicast extends EventEmitter {
  ...
  send(message) {
    ...
    DEBUG(
      `sending ${message} to all servers at ${MULTICAST_ADDRESS}:${DEFAULT_GAME_SERVER_PORT}`
    );
...
class Game {
      ...
      for (let y = new_i - 1; y <= new_i + 1; y++) {
        for (let x = new_j - 1; x <= new_j + 1; x++) {
          if (this.map[y][x] instanceof Zombie) {
            ...
            if (x === new_j && y > new_i)
              this.move(this.map[y][x], "attack", "N");
            if (x === new_j && y < new_i)
              this.move(this.map[y][x], "attack", "S");
            if (y === new_i && x > new_j)
              this.move(this.map[y][x], "attack", "W");
            if (y === new_i && x < new_j)
              this.move(this.map[y][x], "attack", "E");
            if (x > new_j && y > new_i)
              this.move(this.map[y][x], "attack", "NW");
            if (x > new_j && y < new_i)
              this.move(this.map[y][x], "attack", "SW");
            if (x < new_j && y > new_i)
              this.move(this.map[y][x], "attack", "NE");
            if (x < new_j && y < new_i)
              this.move(this.map[y][x], "attack", "SE");
            found = true;
          }
          if (found) break;
        }
      ...
      if (enemy.damaged === false) {
        ...
        if (enemy.health <= 0 && !(enemy instanceof Ground)) {
          ...
          if (enemy instanceof Player) {
            ...
            this.event.emit("happening", {
              type: "kill",
              killer: entity.toString(),
              victim: enemy.toString(),
            });
          }
        } else {
            ...
            data: [
              {
                x: new_j,
                y: new_i,
                what: "d" + this.map[new_i][new_j].toString(),
              },
            ],
          });
  ...
  kill(entity) {
    ...
    this.event.emit("happening", {
      type: "changes",
      data: [{ x: x, y: y, what: "f" + entity.toString() }],
    });
    //kill them
    ...
    setTimeout(() => {
      ...
      this.event.emit("happening", {
        type: "changes",
        data: [{ x: x, y: y, what: this.map[y][x].toString() }],
      });
    }, INVINCIBILITY_FRAMES);
  }
}
...
function start_game() {
  ...
  game_server.on("connection", (socket) => {
    ...
    game.event.on("log", (message_to_log) => {
      ...
      if (!socket.player) return;
      socket.write(
        JSON.stringify({ type: "log", data: message_to_log }) + DELIMITER
      );
    });
  ...
  game_server.on("close", (socket) => {
    ...
    game.players = game.players.filter(
      (player) => player.number !== socket.player.number
    );
  ...
  game_server.on("message", (message, socket) => {
    ...
    switch (message.type) {
      case "map":
        socket.write(
          JSON.stringify({ type: "map", data: game.string_map() }) + DELIMITER
        );
        break;
      case "join":
  ...
  constructor(connect_to_port, connect_to_address, client_type = "Client") {
    ...
    this.socket.on("data", (data) => {
      ...
      data
        .split(DELIMITER)
        .slice(0, -1)
        .forEach((task) => this.emit("message", JSON.parse(task)));
    });
  }
...
class Display {
  ...
  show(index) {
    ...
    const selected_index =
      index ?? this.current_options[this.current_index]
        ? this.current_index
        : 0;
    ...
    options.forEach((option, index) => {
      ...
      if (option.id === "quit" && index === selected_index) {
        process.stdout.write(
          `${this.COLORS.pure_white}> ${option.name}.         ${this.COLORS.reset}\r`
        );

        let i = 2;
        this.op_animation = setInterval(() => {
          process.stdout.write(
            `${this.COLORS.pure_white}> ${
              options[selected_index].name
            }${".".repeat(i)}         ${this.COLORS.reset}\r`
          );
          if (i === 3) i = 0;
          else i++;
        }, 500);
        ...
        if (index === selected_index) {
          console.log(
            this.COLORS.pure_white + "> " + option.name + this.COLORS.reset
          ); //highlight selected
          if (option.message) {
            process.stdout.write("  "); //margin
            console.log(
              this.COLORS.message + option.message + this.COLORS.reset
            );
          }
        } else {
      ...
      //on key press
        ...
        if (options[selected_index].type === "input") {
          ...
          if (key.name === "backspace") {
            ...
            options[selected_index].value = options[selected_index].value.slice(
              0,
              -1
            );
          } else {
            ...
            if (
              ...
              (key.sequence.charCodeAt(0) >= 127 &&
                key.sequence.charCodeAt(0) <= 159)
            ) {
  ...
  async intro(message, next_menu = "main") {
    ...
    for (let i = 0; i < message.length; i++) {
      ...
      process.stdout.write(
        `\x1b[38;5;${Math.min(
          Math.max(255 - message.length, 232) +
            Math.round(i / (message.length / 23)),
          255
        )}m${displayedString.join("")}`
      );
  ...
  update(data) {
    ...
    data.forEach((change) => {
      for (let k = 0; k < TILE_SIZE; k++) {
        ...
        process.stdout.write(
          this.process_char(change.what).repeat(TILE_SIZE + 1)
        );
      }
    });
  ...
  player_connect() {
    ...
    this.player_client.on("message", (message) => {
      ...
      console.log(
        color + this.render_health(message.player?.health) + this.COLORS.reset
      );
      console.table(this.scoreboard);
      this.to_log.forEach((message) =>
        console.log(message + "                        ")
      );
    });
